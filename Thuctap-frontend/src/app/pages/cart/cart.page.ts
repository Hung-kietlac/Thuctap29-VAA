...
export class CartPage implements OnInit {
  ...
  pay() {
    ...
    const paymentMethod = encodeURIComponent(this.selectedPayment);
    ...
    if (this.selectedPayment === 'vnpay') {
      const apiUrl = `http://127.0.0.1:8000/api/generate_vnpay_qr/?amount=${amount}&phim=${phim}&rap=${rap}&method=${paymentMethod}`;
